...
summary: 平凯数据库中 FLASHBACK CLUSTER TO TIMESTAMP 的使用概况。
---
...
> **警告：**
>
> 在 v7.1.0 中使用该功能可能会出现 FLASHBACK 完成后部分 Region 仍处于 FLASHBACK 过程中的问题。请尽量避免在 v7.1.0 中使用该功能。详情可见 [#44292](https://github.com/pingcap/tidb/issues/44292)。
>
> 如果已经出现该问题，可以使用 平凯数据库快照备份与恢复功能进行数据恢复。
...
* 在 `FLASHBACK CLUSTER` 执行期间，若有数据备份需求，只支持使用 BR 命令行工具进行快照备份，并需要指定早于 `FLASHBACK CLUSTER` 开始时间的 `BackupTS`。同时，在执行 `FLASHBACK CLUSTER` 期间，开启日志备份的操作会失败，请等待 `FLASHBACK CLUSTER` 结束后再尝试开启日志备份。
