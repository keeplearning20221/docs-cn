---
title: ADD INDEX
summary: 平凯数据库中 ADD INDEX 的使用概况。
---

# ADD INDEX

`ALTER TABLE.. ADD INDEX` 语句用于在已有表中添加一个索引。在平凯数据库中，`ADD INDEX` 为在线操作，不会阻塞表中的数据读写。

> **警告：**
>
> - 在升级平凯数据库集群的过程中，**请勿执行** DDL 语句，否则可能会出现行为未定义的问题。
...
> 
> 从 v7.1 版本升级至更高的版本时，可以不遵循以上限制，建议参考平滑升级平凯数据库的限制。
